...
|---|---|---|---|
| OpenAI Chat Completions | [openai-chat-completions.md](openai-chat-completions.md) | inbound + upstream | medium — annotations + audio-usage + streaming-usage gaps |
| OpenAI Responses | [openai-responses.md](openai-responses.md) | inbound + upstream (byte-pass when host is openai-proper) | medium — reasoning signature + streaming refusal/tool gaps |
| Anthropic Messages | [anthropic.md](anthropic.md) | inbound + upstream | medium — streaming thinking-signature loss + max_tokens default |
| Google Gemini | [gemini.md](gemini.md) | **upstream only** (new, undogfooded) | low-confidence — structured-output drop, streaming args bug |
...
- **Anthropic**: hardcodes `4096` when canonical `MaxTokens` is nil (Anthropic requires the field) — silently caps 64k-output models (`translator_canonical.go:32,354`).

## Severity-ranked action list

**P0 — correctness bugs (wrong/corrupt output or panic):**
...
**P1 — silent data loss / observability gaps:**
...
12. **Anthropic** `max_tokens` 4096 silent cap.
