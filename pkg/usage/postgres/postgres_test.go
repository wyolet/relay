...
func TestIntegration_RoundTrip(t *testing.T) {
	...
	// Unique marker so repeated runs don't interfere.
	...
	// Summary aggregation.
	res, err := s.Summary(ctx, usage.SummaryQuery{
	...
	byLeg, err := s.Summary(ctx, usage.SummaryQuery{EventQuery: q, GroupBy: "tags.leg"})
	if err != nil {
	...
	if p3, err := s.Events(ctx, pq); err != nil || len(p3) != 0 {
